...
import { Smartphone, Wifi, Globe2, ExternalLink, Check } from 'lucide-react';
            ...
            <div className="lg:col-span-4 flex justify-center">
              <div className="w-full max-w-xs rounded-2xl bg-[#08201A] border border-emerald-500/40 p-6 shadow-xl text-center space-y-4">
                ...
                <div>
                  ...
                  <h4 className="text-lg font-bold text-white font-serif">{t('airalo.cardTitle')}</h4>
                  <p className="text-xs text-emerald-200/80 mt-1">{t('airalo.cardSpeed')}</p>
                </div>
                <div className="p-3 rounded-xl bg-slate-900/90 border border-emerald-700/60 text-xs text-slate-200 space-y-1.5">
                  <div className="flex justify-between">
                    <span className="text-slate-400">{t('airalo.coverage')}</span>
                    <span className="font-semibold text-white">{t('airalo.coverageValue')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">{t('airalo.setup')}</span>
                    <span className="font-semibold text-emerald-400">{t('airalo.setupValue')}</span>
                  </div>
                </div>
                <button
                  onClick={handleGetEsim}
